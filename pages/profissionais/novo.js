...
export default function NovoProfissionalPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast({ title: "Sucesso", description: "Profissional criado com sucesso!", variant: "success" });
      router.push("/profissionais");
    } catch (err) {
      toast({ title: "Erro", description: "Falha ao criar profissional.", variant: "error" });
    } finally {
      setSaving(false);
    }
  }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900">Novo Profissional</h1>
              <p className="text-sm text-slate-500">Cadastre um novo colaborador</p>
            </div>
          </div>
        </CardHeader>
        <CardBody>
          ...
          <div className="flex justify-end gap-2 pt-6 mt-6 border-t border-slate-200">
            ...
            <Button loading={saving} icon={<Save className="h-4 w-4" />} onClick={handleSubmit}>
              Salvar profissional
            </Button>
          </div>
        </CardBody>
...
NovoProfissionalPage.getLayout = function getLayout(page) {
