...
export default function EditarServicoPage() {
  ...
  async function loadServico() {
    ...
    try {
      setLoading(true);
      const data = await serviceService.get(id);
      setForm({
        name: data.name || "",
      ...
      toast({ title: "Erro", description: "Falha ao carregar serviço.", variant: "error" });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast({ title: "Sucesso", description: "Serviço atualizado com sucesso!", variant: "success" });
      router.push("/servicos");
    } catch (err) {
      toast({ title: "Erro", description: "Falha ao atualizar serviço.", variant: "error" });
    } finally {
      setSaving(false);
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900">
                {loading ? <Skeleton className="h-6 w-40 inline-block" /> : "Editar Serviço"}
              </h1>
              <p className="text-sm text-slate-500">Atualize os dados do serviço</p>
            </div>
          </div>
        </CardHeader>
        <CardBody>
...
EditarServicoPage.getLayout = function getLayout(page) {
